...
})(BridgeOperationType || (exports.BridgeOperationType = BridgeOperationType = {}));
/**
...
class CrossChainBridge {
    constructor(minValidatorSignatures = 3) {
        ...
        this.bridgeContracts.set(src_1.ChainId.STELLAR, "GBRIDGECONTRACT123EXAMPLE");
        this.bridgeContracts.set(src_1.ChainId.STARKNET, "0x1234567890abcdef1234567890abcdef12345678");
    }
    /**
    ...
    addValidator(validator) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const provider = new src_1.MockSignatureProvider(validator.providerId, undefined, {
                enableLogging: true,
                customCapabilities: {
            ...
            console.log(`✅ Validator ${validator.validatorId} added for chains: ${validator.supportedChains.join(", ")}`);
        });
    }
    /**
    ...
    executeBridgeOperation(request) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            console.log(`   From: ${request.sourceChain} → To: ${request.destinationChain}`);
            ...
            try {
                ...
                const bridgeProof = yield this.generateBridgeProof(request, sourceResult.transactionHash);
                // Step 3: Execute destination chain transaction
                const destinationResult = yield this.executeDestinationChainTransaction(request, bridgeProof);
                return {
                    ...
                    validatorSignatures: sourceResult.validatorSignatures +
    ...
    executeSourceChainTransaction(request) {
        return __awaiter(this, void 0, void 0, function* () {
            console.log(`📤 Executing source chain transaction on ${request.sourceChain}`);
            ...
            const transaction = this.createChainTransaction(request, request.sourceChain);
            try {
                const workflowResult = yield coordinator.startWorkflow({ chainId: request.sourceChain, transaction }, signers, config);
                if (workflowResult.status === "completed" && workflowResult.requiredMet) {
                    return {
                        ...
                        validatorSignatures: workflowResult.signatures.filter((s) => s.status === "completed").length,
                    };
                }
                else {
                    return {
    ...
    executeDestinationChainTransaction(request, bridgeProof) {
        return __awaiter(this, void 0, void 0, function* () {
            console.log(`📥 Executing destination chain transaction on ${request.destinationChain}`);
            ...
            const proofValid = yield this.verifyBridgeProof(bridgeProof, request.sourceChain, request.destinationChain, request.amount);
            if (!proofValid) {
                return {
                    success: false,
            ...
            const destinationRequest = Object.assign(Object.assign({}, request), { operationType: request.operationType === BridgeOperationType.LOCK
    ...
    getValidatorsForChain(chainId) {
        return Array.from(this.validators.values()).filter((validator) => validator.supportedChains.includes(chainId));
    }
        ...
        switch (chainId) {
            ...
            case src_1.ChainId.STELLAR:
                return {
                    ...
                    operations: [
                        {
                            type: request.operationType === BridgeOperationType.LOCK
                                ? "payment"
                                : "create_account",
        ...
        try {
            ...
            const validators = [
                ...
                },
                {
                    ...
                    supportedChains: [src_1.ChainId.BITCOIN, src_1.ChainId.STELLAR, src_1.ChainId.STARKNET],
            ...
            const btcToStellarResult = yield bridge.executeBridgeOperation(btcToStellarRequest);
            console.log("Bridge Result:", btcToStellarResult.success ? "✅ Success" : "❌ Failed");
            if (btcToStellarResult.success) {
                console.log(`Source TX: ${btcToStellarResult.sourceTransactionHash}`);
                console.log(`Destination TX: ${btcToStellarResult.destinationTransactionHash}`);
                console.log(`Validator Signatures: ${btcToStellarResult.validatorSignatures}/${btcToStellarResult.requiredSignatures}`);
            }
            else {
            ...
            const stellarToStarknetResult = yield bridge.executeBridgeOperation(stellarToStarknetRequest);
            console.log("Bridge Result:", stellarToStarknetResult.success ? "✅ Success" : "❌ Failed");
            if (stellarToStarknetResult.success) {
                console.log(`Source TX: ${stellarToStarknetResult.sourceTransactionHash}`);
                console.log(`Destination TX: ${stellarToStarknetResult.destinationTransactionHash}`);
                console.log(`Validator Signatures: ${stellarToStarknetResult.validatorSignatures}/${stellarToStarknetResult.requiredSignatures}`);
            }
            else {
            ...
            if (btcToStellarResult.success && btcToStellarResult.bridgeProof) {
                const proofValid = yield bridge.verifyBridgeProof(btcToStellarResult.bridgeProof, src_1.ChainId.BITCOIN, src_1.ChainId.STELLAR, "0.1");
                console.log("Bridge Proof Valid:", proofValid ? "✅ Valid" : "❌ Invalid");
            }
        }
        catch (error) {
