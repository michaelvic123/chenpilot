...
exports.sorobanValidator = exports.horizonValidator = exports.SchemaValidationError = void 0;
 ...
 */
function validateField(data, field, expectedType, required = true) {
...
function validateHorizonTransaction(data) {
    ...
    if (dataObj.successful !== undefined &&
 ...
 */
function validateAccountOffer(data) {
    const errors = [];
 ...
 */
function validateSorobanTransaction(data) {
    const errors = [];
    const statusCheck = validateField(data, "status", "string", true);
    if (statusCheck)
        errors.push(statusCheck);
    const dataObj = data;
 ...
 */
function validateJsonRpcBatchResponse(data) {
    const errors = [];
...
exports.horizonValidator = {
     ...
     */
    throwOnInvalidTransaction(data) {
        const result = this.validateTransaction(data);
        if (!result.valid) {
            throw new SchemaValidationError("Invalid Horizon transaction response", result.errors);
        }
        return data;
    },
    /**
     ...
     */
    throwOnInvalidAccountOffer(data) {
        const result = this.validateAccountOffer(data);
        if (!result.valid) {
            throw new SchemaValidationError("Invalid Horizon account offer response", result.errors);
        }
        return data;
    },
};
...
exports.sorobanValidator = {
    ...
    throwOnInvalidTransaction(data) {
        const result = this.validateTransaction(data);
        if (!result.valid) {
            throw new SchemaValidationError("Invalid Soroban RPC transaction response", result.errors);
        }
        return data;
    },
    /**
     ...
     */
    throwOnInvalidBatchResponse(data) {
        const result = this.validateBatchResponse(data);
        if (!result.valid) {
            throw new SchemaValidationError("Invalid Soroban RPC batch response", result.errors);
        }
        return data;
    },
};
