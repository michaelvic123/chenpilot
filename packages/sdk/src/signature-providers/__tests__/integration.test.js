...
describe("SignatureProvider Integration Tests", () => {
    ...
    describe("End-to-End Provider Workflow", () => {
        it("should complete full provider lifecycle", () => __awaiter(void 0, void 0, void 0, function* () {
            ...
            const verification = yield signature_verification_1.SignatureVerificationUtils.verifySignature(verificationRequest);
            expect(verification.chainId).toBe(types_1.ChainId.STELLAR);
            // 7. Disconnect and cleanup
            yield provider.disconnect();
            expect(provider.isConnected()).toBe(false);
        }));
        it("should handle multi-chain workflow", () => __awaiter(void 0, void 0, void 0, function* () {
            // Create providers for different chains
            const bitcoinProviders = yield factory.createProvidersForChain(types_1.ChainId.BITCOIN, {
                autoConnect: true,
                autoRegister: true,
            });
            ...
            const starknetProviders = yield factory.createProvidersForChain(types_1.ChainId.STARKNET, {
                autoConnect: true,
                autoRegister: true,
            });
    ...
    describe("Multi-Signature Integration", () => {
        it("should complete multi-signature workflow", () => __awaiter(void 0, void 0, void 0, function* () {
            // Create multiple providers for multi-sig
            const mockProvider1 = new mock_provider_1.MockSignatureProvider("multisig-provider-1");
            const mockProvider2 = new mock_provider_1.MockSignatureProvider("multisig-provider-2");
            const mockProvider3 = new mock_provider_1.MockSignatureProvider("multisig-provider-3");
            ...
            const accounts1 = yield mockProvider1.getAccounts(types_1.ChainId.STELLAR);
            const accounts2 = yield mockProvider2.getAccounts(types_1.ChainId.STELLAR);
            const accounts3 = yield mockProvider3.getAccounts(types_1.ChainId.STELLAR);
            ...
            const workflowResult = yield coordinator.startWorkflow(transactionData, signers, config);
            expect(workflowResult.status).toBe("completed");
            expect(workflowResult.requiredMet).toBe(true);
            expect(workflowResult.signatures.filter((s) => s.status === multi_signature_coordinator_1.SignatureStatus.COMPLETED).length).toBeGreaterThanOrEqual(2);
            expect(workflowResult.finalTransaction).toBeDefined();
            // Verify multi-signature result
            const completedSignatures = workflowResult.signatures.filter((s) => s.status === multi_signature_coordinator_1.SignatureStatus.COMPLETED);
            const multiSigVerification = yield signature_verification_1.SignatureVerificationUtils.verifyMultiSignature(completedSignatures.map((s) => ({
                signature: s.signature,
                publicKey: s.signerInfo.publicKey,
            })), transactionData, types_1.ChainId.STELLAR, config.requiredSignatures);
            expect(multiSigVerification.thresholdMet).toBe(true);
            expect(multiSigVerification.validSignatures.length).toBeGreaterThanOrEqual(config.requiredSignatures);
            // Cleanup
            yield mockProvider1.disconnect();
            yield mockProvider2.disconnect();
            yield mockProvider3.disconnect();
        }));
        it("should handle multi-signature with failures", () => __awaiter(void 0, void 0, void 0, function* () {
            // Create providers with one that will fail
            const mockProvider1 = new mock_provider_1.MockSignatureProvider("fail-provider-1");
            const mockProvider2 = new mock_provider_1.MockSignatureProvider("fail-provider-2", undefined, {
                shouldFailSigning: true,
            });
            const mockProvider3 = new mock_provider_1.MockSignatureProvider("fail-provider-3");
            coordinator.registerProvider(mockProvider1);
            coordinator.registerProvider(mockProvider2);
            coordinator.registerProvider(mockProvider3);
            yield mockProvider1.connect();
            ...
            expect(workflowResult.requiredMet).toBe(true);
            const completedSignatures = workflowResult.signatures.filter((s) => s.status === multi_signature_coordinator_1.SignatureStatus.COMPLETED);
            const failedSignatures = workflowResult.signatures.filter((s) => s.status === multi_signature_coordinator_1.SignatureStatus.FAILED);
    ...
    describe("Provider Registry Integration", () => {
        it("should manage provider lifecycle through registry", () => __awaiter(void 0, void 0, void 0, function* () {
            ...
            const stellarProviders = registry.findProvidersForChain(types_1.ChainId.STELLAR);
            expect(stellarProviders.length).toBeGreaterThan(0);
            const bitcoinProviders = registry.findProvidersForChain(types_1.ChainId.BITCOIN);
            expect(bitcoinProviders.length).toBeGreaterThan(0);
        ...
        it("should handle provider registration events", () => __awaiter(void 0, void 0, void 0, function* () {
            ...
            const provider1 = yield factory.createProvider({ type: provider_factory_1.ProviderType.MOCK }, {
                autoRegister: true,
            });
            const provider2 = yield factory.createProvider({ type: provider_factory_1.ProviderType.LEDGER }, {
                autoRegister: true,
            });
    ...
    describe("Error Handling Integration", () => {
        it("should handle provider connection failures gracefully", () => __awaiter(void 0, void 0, void 0, function* () {
            // Create provider that will fail to connect
            const provider = new mock_provider_1.MockSignatureProvider("failing-provider", undefined, {
                shouldFailConnection: true,
            });
            yield expect(provider.connect()).rejects.toThrow(errors_1.ConnectionError);
        ...
        it("should handle signing failures in multi-signature workflow", () => __awaiter(void 0, void 0, void 0, function* () {
            const mockProvider1 = new mock_provider_1.MockSignatureProvider("error-provider-1");
            const mockProvider2 = new mock_provider_1.MockSignatureProvider("error-provider-2", undefined, {
            ...
            const workflowResult = yield coordinator.startWorkflow({ chainId: types_1.ChainId.STELLAR, transaction }, signers, config);
            expect(workflowResult.status).toBe("completed");
            const rejectedSignatures = workflowResult.signatures.filter((s) => s.status === multi_signature_coordinator_1.SignatureStatus.REJECTED);
            expect(rejectedSignatures.length).toBe(1);
            expect(rejectedSignatures[0].error).toBeInstanceOf(errors_1.UserRejectedError);
            // Cleanup
            yield mockProvider1.disconnect();
            yield mockProvider2.disconnect();
        }));
    ...
    describe("Performance and Scalability", () => {
        it("should handle multiple concurrent operations", () => __awaiter(void 0, void 0, void 0, function* () {
            // Create multiple providers
            const providers = yield factory.createProviders([
                { type: provider_factory_1.ProviderType.MOCK, config: { signingDelay: 100 } },
                { type: provider_factory_1.ProviderType.MOCK, config: { signingDelay: 150 } },
                { type: provider_factory_1.ProviderType.MOCK, config: { signingDelay: 200 } },
            ], {
                autoConnect: true,
            });
            // Perform concurrent signing operations
            const signingPromises = providers.map((provider, index) => __awaiter(void 0, void 0, void 0, function* () {
                const accounts = yield provider.getAccounts(types_1.ChainId.STELLAR);
                const transaction = {
                    sourceAccount: accounts[0].address,
                    fee: "100",
        ...
        it("should handle batch signature verification", () => __awaiter(void 0, void 0, void 0, function* () {
            const provider = new mock_provider_1.MockSignatureProvider("batch-provider");
            ...
            const startTime = Date.now();
            const verificationResults = yield signature_verification_1.SignatureVerificationUtils.batchVerifySignatures(verificationRequests);
            const endTime = Date.now();
            expect(verificationResults.length).toBe(10);
            expect(verificationResults.every((r) => r.chainId === types_1.ChainId.STELLAR)).toBe(true);
            console.log(`Batch verification of 10 signatures took ${endTime - startTime}ms`);
            yield provider.disconnect();
        }));
    });
