...
class SignatureProviderUtils {
     ...
     */
    static validateProviderCapability(provider, chainId, requiresMessageSigning = false) {
        ...
        if (requiresMessageSigning && !capabilities.supportsMessageSigning) {
            throw new errors_1.UnsupportedOperationError("message signing", provider.providerId);
        }
    }
    /**
    ...
    static compareProviders(a, b) {
        ...
        const chainDiff = bCaps.supportedChains.length - aCaps.supportedChains.length;
        if (chainDiff !== 0)
            return chainDiff;
        // Then by concurrent signature capacity
        const concurrentDiff = bCaps.maxConcurrentSignatures - aCaps.maxConcurrentSignatures;
        if (concurrentDiff !== 0)
            return concurrentDiff;
    ...
    static getCapabilityDescription(capabilities) {
        const parts = [];
        parts.push(`Supports ${capabilities.supportedChains.length} chain(s): ${capabilities.supportedChains.join(", ")}`);
        ...
        parts.push(`Max concurrent signatures: ${capabilities.maxConcurrentSignatures}`);
        return parts.join("; ");
    }
    /**
        ...
        return providers.filter((provider) => {
            ...
            if (criteria.supportedChains) {
                if (criteria.requiresAllChains) {
                    ...
                    if (!criteria.supportedChains.every((chain) => capabilities.supportedChains.includes(chain))) {
                        return false;
                    }
                }
                else {
                    // Provider must support AT LEAST ONE specified chain
                    if (!criteria.supportedChains.some((chain) => capabilities.supportedChains.includes(chain))) {
                        return false;
                    }
                }
            }
            ...
            if (criteria.minConcurrentSignatures !== undefined) {
                if (capabilities.maxConcurrentSignatures <
    ...
    static findBestProvider(providers, chainId, preferences = {}) {
        ...
        const compatibleProviders = providers.filter((provider) => this.supportsChain(provider, chainId));
        ...
        const scoredProviders = compatibleProviders.map((provider) => {
            ...
            if (preferences.preferNoUserInteraction &&
                !capabilities.requiresUserInteraction) {
                score += 3;
            }
            if (preferences.preferMessageSigning &&
                capabilities.supportsMessageSigning) {
                score += 2;
            }
            if (preferences.preferMultipleAccounts &&
