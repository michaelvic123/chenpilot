...
exports.MultiSignatureCoordinator = exports.MultiSignatureEventType = exports.SignatureStatus = void 0;
...
})(MultiSignatureEventType || (exports.MultiSignatureEventType = MultiSignatureEventType = {}));
/**
...
class MultiSignatureCoordinator {
    ...
    isThresholdMet(workflow) {
        const completedSignatures = workflow.signatures.filter((sig) => sig.status === SignatureStatus.COMPLETED).length;
    ...
    getSigningProgress(workflow) {
        const completed = workflow.signatures.filter((sig) => sig.status === SignatureStatus.COMPLETED).length;
    ...
    validateWorkflowConfig(config, signers) {
        if (config.requiredSignatures <= 0) {
            throw new errors_1.InvalidTransactionError("Required signatures must be greater than 0");
        }
        if (config.requiredSignatures > config.totalSigners) {
            throw new errors_1.InvalidTransactionError("Required signatures cannot exceed total signers");
        }
        if (signers.length !== config.totalSigners) {
            throw new errors_1.InvalidTransactionError("Number of signers must match total signers in config");
        }
        const requiredSigners = signers.filter((s) => s.required).length;
        if (requiredSigners > config.requiredSignatures) {
            throw new errors_1.InvalidTransactionError("Number of required signers cannot exceed required signatures");
        }
        // Validate that all providers are registered
        for (const signer of signers) {
            if (!this.providers.has(signer.providerId)) {
                throw new errors_1.ConnectionError(`Provider ${signer.providerId} not registered`);
            }
        }
    }
    ...
    executeSequentialSigning(transactionData, workflow) {
        return __awaiter(this, void 0, void 0, function* () {
            for (let i = 0; i < workflow.signatures.length; i++) {
                ...
                if (signatureResult.status === SignatureStatus.FAILED &&
                    !workflow.config.continueOnError) {
                    throw new errors_1.SigningError(`Sequential signing failed at signer ${i + 1}`);
                }
            }
        });
    }
    executeParallelSigning(transactionData, workflow) {
        return __awaiter(this, void 0, void 0, function* () {
            const signingPromises = workflow.signatures.map((signatureResult) => this.executeSignature(transactionData, signatureResult, workflow));
    ...
    executeSignature(transactionData, signatureResult, workflow) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (!provider) {
                ...
                signatureResult.error = new errors_1.ConnectionError(`Provider ${signerInfo.providerId} not found`);
                return;
            }
            ...
            try {
                ...
                const request = {
                    ...
                    metadata: Object.assign(Object.assign({ multiSignature: true, transactionId: workflow.transactionId, signerIndex: workflow.signatures.indexOf(signatureResult) }, workflow.config.metadata), signerInfo.metadata),
                };
                // Execute signature with timeout
                const result = yield this.executeWithTimeout(provider.signTransaction(request), workflow.config.signatureTimeout || 30000);
                ...
                signatureResult.error =
                    error instanceof errors_1.SignatureProviderError
                        ? error
                        : new errors_1.SigningError(`Signature failed: ${error}`, signerInfo.providerId, transactionData.chainId);
                this.emitEvent({
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (workflow.requiredMet) {
                ...
                if (workflow.status === "failed") {
                    throw new errors_1.SigningError(`Multi-signature workflow failed: insufficient signatures (${this.getSigningProgress(workflow).completed}/${workflow.config.requiredSignatures})`);
                }
            }
        });
    }
    combineSignedTransactions(workflow) {
        const completedSignatures = workflow.signatures.filter((sig) => sig.status === SignatureStatus.COMPLETED);
        ...
        return Object.assign(Object.assign({}, firstSigned), { multiSignature: {
