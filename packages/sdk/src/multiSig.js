...
class MultiSigBuilder {
    ...
    constructor(masterAccount, options = {}) {
        ...
        this.options = {
            autoValidate: (_a = options.autoValidate) !== null && _a !== void 0 ? _a : true,
            allowDuplicates: (_b = options.allowDuplicates) !== null && _b !== void 0 ? _b : false,
    ...
    validate() {
        ...
        if (this.signers.length > this.options.maxSigners) {
            errors.push(`Too many signers: ${this.signers.length} (max: ${this.options.maxSigners})`);
        }
        ...
        const totalWeight = this.signers.reduce((sum, signer) => sum + signer.weight, 0);
        // Validate thresholds against total weight
        if (this.thresholds.low > totalWeight) {
            errors.push(`Low threshold (${this.thresholds.low}) exceeds total weight (${totalWeight})`);
        }
        if (this.thresholds.medium > totalWeight) {
            errors.push(`Medium threshold (${this.thresholds.medium}) exceeds total weight (${totalWeight})`);
        }
        if (this.thresholds.high > totalWeight) {
            errors.push(`High threshold (${this.thresholds.high}) exceeds total weight (${totalWeight})`);
        }
    ...
    build() {
        ...
        if (this.options.autoValidate && !validation.valid) {
            throw new Error(`Invalid multi-sig configuration: ${validation.errors.join(", ")}`);
        }
        const config = {
...
function canMeetThreshold(signers, config, threshold) {
    const totalWeight = signers.reduce((sum, address) => {
        ...
        return sum + ((signer === null || signer === void 0 ? void 0 : signer.weight) || 0);
    }, 0);
