...
describe("PlanVerifier", () => {
    ...
    describe("Basic Validation", () => {
        ...
        it("should succeed with valid hash", () => {
            ...
            const planWithHash = Object.assign(Object.assign({}, validPlan), { planHash: hash });
        ...
        it("should detect hash mismatch", () => {
            const planWithHash = Object.assign(Object.assign({}, validPlan), { planHash: "invalid_hash" });
            const result = verifier.verifyPlan(planWithHash);
            expect(result.valid).toBe(false);
            expect(result.hashMatch).toBe(false);
            expect(result.errors.some((e) => e.includes("Plan hash mismatch"))).toBe(true);
    ...
    describe("Signature Verification", () => {
        ...
        it("should fail when signature required but not present", () => {
            ...
            expect(result.valid).toBe(false);
            expect(result.errors).toContain("Signature required but not present in plan");
        });
        it("should warn when signature present but no public key", () => {
            const hash = computeTestHash(validPlan);
            const signature = signHash(hash, privateKey);
            const planWithHashAndSig = Object.assign(Object.assign({}, validPlan), { planHash: hash, signature });
            const result = verifier.verifyPlan(planWithHashAndSig);
            expect(result.valid).toBe(true);
            expect(result.warnings).toContain("Signature present but no public key provided for verification");
        });
        ...
        it("should detect invalid signature", () => {
            const hash = computeTestHash(validPlan);
            const planWithHashAndSig = Object.assign(Object.assign({}, validPlan), { planHash: hash, signature: "invalid_signature" });
            const result = verifier.verifyPlan(planWithHashAndSig, { publicKey });
            expect(result.valid).toBe(false);
    ...
    describe("Strict Mode Validations", () => {
        it("should fail for plan with no steps", () => {
            const emptyPlan = Object.assign(Object.assign({}, validPlan), { steps: [], totalSteps: 0 });
            const hash = computeTestHash(emptyPlan);
            const planWithHash = Object.assign(Object.assign({}, emptyPlan), { planHash: hash });
        ...
        it("should warn for plan with many steps", () => {
            ...
            const largePlan = Object.assign(Object.assign({}, validPlan), { steps: manySteps, totalSteps: 60 });
            const hash = computeTestHash(largePlan);
            const planWithHash = Object.assign(Object.assign({}, largePlan), { planHash: hash });
            const result = verifier.verifyPlan(planWithHash, { strictMode: true });
            expect(result.warnings).toContain("Plan has unusually high number of steps (>50)");
        });
        it("should detect duplicate step numbers", () => {
            const duplicatePlan = Object.assign(Object.assign({}, validPlan), { steps: [
                    {
                        stepNumber: 1,
                        action: "action1",
                        payload: {},
            ...
            const planWithHash = Object.assign(Object.assign({}, duplicatePlan), { planHash: hash });
            const result = verifier.verifyPlan(planWithHash, { strictMode: true });
            expect(result.valid).toBe(false);
            expect(result.errors.some((e) => e.includes("Duplicate step numbers"))).toBe(true);
        });
        it("should detect missing step numbers", () => {
            const gappedPlan = Object.assign(Object.assign({}, validPlan), { steps: [
                    {
                        stepNumber: 1,
                        action: "action1",
                        payload: {},
            ...
            const planWithHash = Object.assign(Object.assign({}, gappedPlan), { planHash: hash });
            const result = verifier.verifyPlan(planWithHash, { strictMode: true });
            expect(result.valid).toBe(false);
            expect(result.errors).toContain("Missing step number: 2");
        });
        it("should detect step count mismatch", () => {
            const mismatchPlan = Object.assign(Object.assign({}, validPlan), { steps: [
                    {
                        stepNumber: 1,
                        action: "action1",
                        payload: {},
            ...
            const planWithHash = Object.assign(Object.assign({}, mismatchPlan), { planHash: hash });
            const result = verifier.verifyPlan(planWithHash, { strictMode: true });
            expect(result.valid).toBe(false);
            expect(result.errors.some((e) => e.includes("Step count mismatch"))).toBe(true);
        });
        it("should detect suspicious patterns", () => {
            const suspiciousPlan = Object.assign(Object.assign({}, validPlan), { steps: [
                    {
                        stepNumber: 1,
            ...
            const planWithHash = Object.assign(Object.assign({}, suspiciousPlan), { planHash: hash });
            const result = verifier.verifyPlan(planWithHash, { strictMode: true });
    ...
    describe("Plan Comparison", () => {
        ...
        it("should detect plan ID change", () => {
            const modifiedPlan = Object.assign(Object.assign({}, validPlan), { planId: "plan_456" });
        ...
        it("should detect action change", () => {
            const modifiedPlan = Object.assign(Object.assign({}, validPlan), { steps: [
                    Object.assign(Object.assign({}, validPlan.steps[0]), { action: "different_action" }),
                    validPlan.steps[1],
                ] });
            const result = verifier.comparePlans(validPlan, modifiedPlan);
            expect(result.identical).toBe(false);
            expect(result.differences.some((d) => d.includes("action changed"))).toBe(true);
        });
        it("should detect payload modification", () => {
            const modifiedPlan = Object.assign(Object.assign({}, validPlan), { steps: [
                    Object.assign(Object.assign({}, validPlan.steps[0]), { payload: Object.assign(Object.assign({}, validPlan.steps[0].payload), { extra: "field" }) }),
                    validPlan.steps[1],
                ] });
            const result = verifier.comparePlans(validPlan, modifiedPlan);
            expect(result.identical).toBe(false);
            expect(result.differences.some((d) => d.includes("payload modified"))).toBe(true);
        });
    });
    describe("Hash Determinism", () => {
        ...
        it("should produce different hash for different plans", () => {
            const plan2 = Object.assign(Object.assign({}, validPlan), { planId: "different_id" });
            const hash1 = computeTestHash(validPlan);
            const hash2 = computeTestHash(plan2);
            expect(hash1).not.toBe(hash2);
        });
        it("should be order-independent for object keys", () => {
            const plan1 = Object.assign(Object.assign({}, validPlan), { steps: [
                    {
                        stepNumber: 1,
                        action: "swap",
                        payload: { a: 1, b: 2, c: 3 },
            ...
            const plan2 = Object.assign(Object.assign({}, validPlan), { steps: [
                    {
                        stepNumber: 1,
                        action: "swap",
                        payload: { c: 3, a: 1, b: 2 },
...
describe("Convenience Functions", () => {
    it("should use planVerifier singleton", () => {
        expect(planVerification_1.planVerifier).toBeInstanceOf(planVerification_1.PlanVerifier);
    });
