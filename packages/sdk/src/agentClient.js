...
exports.createBtcToStellarSwapIdempotencyKey = createBtcToStellarSwapIdempotencyKey;
...
function generateIdempotencyKey({ namespace, payload, clientRequestId, }) {
    const fingerprint = (0, crypto_1.createHash)("sha256")
        ...
        .digest("hex")
        .slice(0, 24);
    const requestId = clientRequestId !== null && clientRequestId !== void 0 ? clientRequestId : (0, crypto_1.randomUUID)();
...
function createTimedSignal(timeoutMs, externalSignal) {
    ...
    if (externalSignal) {
        if (externalSignal.aborted) {
            ...
            (_a = externalSignal.addEventListener) === null || _a === void 0 ? void 0 : _a.call(externalSignal, "abort", () => controller.abort(), {
                once: true,
            });
        }
    }
...
class AgentClient {
    constructor(options) {
        ...
        this.defaultTimeoutMs = (_a = options.defaultTimeoutMs) !== null && _a !== void 0 ? _a : 15000;
        this.defaultMaxRetries = (_b = options.defaultMaxRetries) !== null && _b !== void 0 ? _b : 3;
        this.defaultRetryDelayMs = (_c = options.defaultRetryDelayMs) !== null && _c !== void 0 ? _c : 500;
        const runtimeFetch = globalThis.fetch;
        const selectedFetch = (_d = options.fetchFn) !== null && _d !== void 0 ? _d : runtimeFetch;
        if (!selectedFetch) {
    ...
    query(request) {
        return __awaiter(this, void 0, void 0, function* () {
            var _a, _b, _c, _d;
            const idempotencyKey = (_a = request.idempotencyKey) !== null && _a !== void 0 ? _a : generateIdempotencyKey({
            ...
            const maxRetries = (_b = request.maxRetries) !== null && _b !== void 0 ? _b : this.defaultMaxRetries;
            const timeoutMs = (_c = request.timeoutMs) !== null && _c !== void 0 ? _c : this.defaultTimeoutMs;
            const retryDelayMs = (_d = request.retryDelayMs) !== null && _d !== void 0 ? _d : this.defaultRetryDelayMs;
            let attempts = 0;
                ...
                try {
                    ...
                    if (!response.ok) {
                        ...
                        if (!RETRIABLE_STATUS_CODES.has(response.status) ||
                            attempts >= maxRetries) {
                            throw new AgentRequestError(`Agent query failed: ${message}`, idempotencyKey, attempts, response.status);
                        }
                    ...
                    if (!(isAbort || isNetwork) || attempts >= maxRetries) {
                        throw new AgentRequestError(`Agent query failed: ${lastErrorMessage}`, idempotencyKey, attempts, lastStatusCode);
                    }
                    yield sleep(retryDelayMs * attempts);
                }
                finally {
    ...
    executeBtcToStellarSwap(swapRequest, options) {
        return __awaiter(this, void 0, void 0, function* () {
            var _a;
            if (swapRequest.fromChain !== types_1.ChainId.BITCOIN ||
                swapRequest.toChain !== types_1.ChainId.STELLAR) {
                throw new Error("executeBtcToStellarSwap only supports fromChain=bitcoin and toChain=stellar");
            }
            const idempotencyKey = (_a = options.idempotencyKey) !== null && _a !== void 0 ? _a : createBtcToStellarSwapIdempotencyKey(swapRequest);
            return this.query({
