...
(0, globals_1.describe)("Agent Memory Seeding", () => {
    ...
    (0, globals_1.describe)("seedMemoryData", () => {
        ...
        (0, globals_1.it)("should clear existing data when clearExisting is true", () => {
        ...
        (0, globals_1.it)("should preserve existing data when clearExisting is false", () => {
            // Add some initial data
            memory_1.memoryStore.add("test-agent", "Initial entry");
        ...
        (0, globals_1.it)("should seed performance agent with 100 entries (limited to 10 by max context)", () => {
            (0, seedAgentMemory_1.seedMemoryData)();
            const agentIds = (0, seedAgentMemory_1.getSeededAgentIds)();
    ...
    (0, globals_1.describe)("seedAgentMemory", () => {
        ...
        (0, globals_1.it)("should clear existing agent data when clearExisting is true", () => {
            // Add initial data
            memory_1.memoryStore.add("custom-agent", "Old entry");
            // Seed with clearExisting=true
            (0, seedAgentMemory_1.seedAgentMemory)("custom-agent", ["New entry"], true);
            const memory = memory_1.memoryStore.get("custom-agent");
            (0, globals_1.expect)(memory.length).toBe(1);
            (0, globals_1.expect)(memory[0]).toBe("New entry");
        });
        (0, globals_1.it)("should append to existing data when clearExisting is false", () => {
            // Add initial data
            memory_1.memoryStore.add("custom-agent", "Old entry");
            // Seed with clearExisting=false
            (0, seedAgentMemory_1.seedAgentMemory)("custom-agent", ["New entry"], false);
            const memory = memory_1.memoryStore.get("custom-agent");
    ...
    (0, globals_1.describe)("Memory Retrieval Logic", () => {
        ...
        (0, globals_1.it)("should retrieve Stellar-focused conversations for user1", () => {
            const agentIds = (0, seedAgentMemory_1.getSeededAgentIds)();
            const memory = memory_1.memoryStore.get(agentIds.user1);
            const stellarEntries = memory.filter((entry) => entry.toLowerCase().includes("stellar"));
            (0, globals_1.expect)(stellarEntries.length).toBeGreaterThan(0);
        });
        (0, globals_1.it)("should retrieve DeFi-focused conversations for user2", () => {
            const agentIds = (0, seedAgentMemory_1.getSeededAgentIds)();
            const memory = memory_1.memoryStore.get(agentIds.user2);
            const defiEntries = memory.filter((entry) => entry.toLowerCase().includes("apy") ||
        ...
        (0, globals_1.it)("should handle empty memory for non-existent agent", () => {
        ...
        (0, globals_1.it)("should retrieve conversation context for multi-turn interactions", () => {
            const agentIds = (0, seedAgentMemory_1.getSeededAgentIds)();
            const memory = memory_1.memoryStore.get(agentIds.user1);
    ...
    (0, globals_1.describe)("Performance Testing with Seeded Data", () => {
        ...
        (0, globals_1.it)("should handle concurrent memory operations", () => {
            ...
            for (let i = 0; i < 50; i++) {
                operations.push(Promise.resolve(memory_1.memoryStore.add(agentId, `Concurrent entry ${i}`)));
            }
