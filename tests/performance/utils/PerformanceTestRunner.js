...
class PerformanceTestRunner {
    ...
    runTest(testName_1, operation_1) {
        return __awaiter(this, arguments, void 0, function* (testName, operation, options = {}) {
            const { iterations = 10, warmupIterations = 2, threshold, collectMemory = true, } = options;
            ...
            for (let i = 0; i < iterations; i++) {
                ...
                metrics.push({
                    ...
                    memoryUsage: endMemory
                        ? {
                            heapUsed: endMemory.heapUsed - ((startMemory === null || startMemory === void 0 ? void 0 : startMemory.heapUsed) || 0),
    ...
    checkThresholds(statistics, threshold) {
        ...
        if (threshold.mean && statistics.mean > threshold.mean) {
            violations.push(`Mean duration ${statistics.mean.toFixed(2)}ms exceeds threshold ${threshold.mean}ms`);
        }
        if (threshold.p50 && statistics.median > threshold.p50) {
            violations.push(`P50 duration ${statistics.median.toFixed(2)}ms exceeds threshold ${threshold.p50}ms`);
        }
        if (threshold.p95 && statistics.p95 > threshold.p95) {
            violations.push(`P95 duration ${statistics.p95.toFixed(2)}ms exceeds threshold ${threshold.p95}ms`);
        }
        if (threshold.p99 && statistics.p99 > threshold.p99) {
            violations.push(`P99 duration ${statistics.p99.toFixed(2)}ms exceeds threshold ${threshold.p99}ms`);
        }
        if (threshold.max && statistics.max > threshold.max) {
            violations.push(`Max duration ${statistics.max.toFixed(2)}ms exceeds threshold ${threshold.max}ms`);
        }
    ...
    logResult(result) {
        ...
        if (!passed && violations) {
            logger_1.default.warn(`Performance threshold violations for ${testName}:`, {
                violations,
            });
        }
    }
